...
def make_candidates(items: List[DictItem], idxs: List[int]) -> List[dict]:
    out = []
    for i in idxs:
        it = items[i]
...
def topk_candidates(llm, ui: UserInput, categories: List[DictItem], risks: List[DictItem],
    ...
    cat_idx = cosine_topk(q_vec, cat_mat, min(k, len(categories)))
    risk_idx = cosine_topk(q_vec, risk_mat, min(k, len(risks)))

    return {
        "deviation_categories": make_candidates(categories, cat_idx),
        "risks": make_candidates(risks, risk_idx),
    }
