...
async def build(m: Message):
    ...
    await m.answer("генерирую анализ...")

    # Отдаём все категории и риски Claude для выбора
    candidates = {
        "deviation_categories": [{"id": c.id, "name": c.name} for c in CATEGORIES],
        "risks": [{"id": r.id, "name": r.name} for r in RISKS]
    }
