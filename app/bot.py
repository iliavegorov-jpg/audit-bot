...
def kb_sections(dev_id: int) -> InlineKeyboardBuilder:
    ...
    # разделитель и кнопка нового отклонения
...
async def start(m: Message, state: FSMContext):
    ...
    if is_authorized(user_id):
        await m.answer(
"✅ Добро пожаловать!\n\nДля создания анализа отклонения используйте /new"
        )
        return
...
async def check_password(m: Message, state: FSMContext):
    if m.text.strip() == VALID_PASSWORD:
        ...
        await state.clear()
        await m.answer("✅ Доступ разрешён!\n\nДля создания анализа используйте /new")
    else:
...
async def handle_full_description(m: Message, state: FSMContext):
    ...
    try:
        ...
        await m.answer("⚠️ Данный анализ требует проверки бухгалтером/аудитором.")
        await m.answer("☝️ Для нового анализа нажми кнопку «🆕 НОВОЕ ОТКЛОНЕНИЕ» в меню выше")
